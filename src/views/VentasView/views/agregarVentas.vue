<template>
  <v-container>
            ...
            <v-card-text>
              <v-row>
                <v-col cols="12" class="text-end">
                  <v-tooltip v-model="show" location="bottom">
                    <template v-slot:activator="{ props }">
                      <v-btn icon v-bind="props" density="compact" elevation="0" color="grey-darken-4"
                        @click="closeDialog">
                ...
                <v-col cols="12" md="4" sm="12">
                  <v-combobox density="compact" label="Nombre comercial" variant="outlined" :items="clientes"
                    item-title="nombreComercial" item-value="id" :rounded="'lg'"
                    @update:model-value="onChangeCliente"></v-combobox>
                </v-col>
                <v-col cols="12" md="4" sm="12">
                  <v-text-field density="compact" variant="outlined" label="Dui" :model-value="cliente.dui"
                    disabled></v-text-field>
                </v-col>
                <v-col cols="12" md="4" sm="12">
                  <v-text-field density="compact" variant="outlined" label="Nit" :model-value="cliente.nit"
                    disabled></v-text-field>
                </v-col>
                <v-col cols="12" md="4" sm="12">
                  <v-text-field density="compact" variant="outlined" label="Registro" :model-value="cliente.registro"
                    disabled></v-text-field>
                </v-col>
                <v-col cols="12" md="4" sm="12">
                  <v-text-field density="compact" variant="outlined" label="Dirección" :model-value="cliente.direccion"
                    disabled></v-text-field>
                </v-col>
              </v-row>
            </v-card-text>
      ...
      <v-col>
        ...
        <v-card :variant="'outlined'" color="surface-variant" label="Detalle factura">
          <v-card-item>
            <v-card-text>
              <v-col cols="12">
                <v-btn class="text-none" @click="agregarDetalles" color="grey-darken-4" :disabled="!cliente.id">Agregar
                  detalle venta</v-btn>
              </v-col>
              <v-col cols="12">
                ...
                <v-card :variant="'outlined'" color="surface-variant" label="Detalle factura">
                  <v-card-item>
                    <v-card-text>
                     <v-row>
                      <v-col cols="12" sm="7">
                        <v-file-input variant="outlined" density="compact" label="Imagen" rounded="lg"
                        @update:model-value="onImageChange"></v-file-input>
                      </v-col>
                      <v-col cols="11" sm="5">
                        <v-text-field :model-value="fechaFacturacionFormated" label="Fecha de facturación" density="compact" variant="outlined"
    ...
    <modal-agregar-detalle-ventas :add-detalle-venta="dialogDetail" @update:add-detalle-ventas-dialog="AddDetalleVentas"
